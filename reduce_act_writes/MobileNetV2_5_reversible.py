...
if __name__ == '__main__':
    ...
    for block_idx,block in enumerate(net.features):
        try:
            ...
            block_end_layer.append(len(weight)-1)
            # size of weight_size
            ...
            for subblock_idx, subblock in enumerate(block.conv):
                ...
                print(4*sum([para.nelement() for para in subblock.parameters()]),x.nelement()*4)
            ...
            print(4*sum([para.nelement() for para in block.parameters()]),x.nelement()*4)
    ...
    act.append(x.nelement()*4)
    print('classifier',net.classifier,4*sum([para.nelement() for para in net.classifier.parameters()]),x.nelement()*4)
    ...
    for layer_index in range(len(act))[-1::-1]:
        if layer_index in block_end_layer:
            ...
            print([round(x) for x in act[block_end_layer[block_end_layer.index(layer_index)-1]:layer_index]],layer_index)
            memory_backward.append(memory_backward[-1]+new_act)
        elif layer_index<max(block_end_layer):
            memory_backward.append(memory_backward[-1]-act[layer_index])
            print(act[layer_index],layer_index)

    for new_read in zip(act[-1::-1],weight[-1::-1],act[-2::-1]):
        # read gradient of output, weight, and input
        read_backward.append(sum(new_read))
        #also the activations and weight are read once again to compute
        read_backward.append(new_read[0]+new_read[1])


    for new_write in zip(act[-1::-1],act[-1::-1],weight[-1::-1]):
