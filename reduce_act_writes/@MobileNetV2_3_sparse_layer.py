...
# specific: update which layer's weight and bias. 
    ...
    layer_update_weight = [45,50,57,62,68,75,86,93,len(weight)-2,len(weight)-1]
    layer_update_bias = list(range(len(weight)//2,len(weight)))
    ...
    for layer_index in range(len(act))[-1::-1]:
        new_read = [act[layer_index],act[layer_index],weight[layer_index],act[layer_index-1]]
        if layer_index in layer_update_weight:
    ...
    print(flops_forward+flops_backward,sum(flops_forward+flops_backward))
